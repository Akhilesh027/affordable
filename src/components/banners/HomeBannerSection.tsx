...
const DEFAULT_AFFORDABLE_BANNERS = {
  videoBanner: {
...
export const HomeBannerSection = () => {
  const [videoBanner, setVideoBanner] = useState<VideoBanner>(DEFAULT_AFFORDABLE_BANNERS.videoBanner);
  const [dualBanners, setDualBanners] = useState<DualBannerItem[]>(DEFAULT_AFFORDABLE_BANNERS.dualBanners);

  useEffect(() => {
    let isMounted = true;
    const loadBanners = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/public/banners/affordable?t=${Date.now()}`);
        if (!res.ok) throw new Error("Could not fetch banners");
  ...
  return (
    <section className="w-full bg-background pt-3 pb-6 space-y-4 md:space-y-6">
      <div className="container mx-auto px-4 space-y-4 md:space-y-6">
        ...
        {videoBanner.isActive !== false && videoBanner.videoUrl && (
          <div className="relative w-full rounded-2xl md:rounded-3xl overflow-hidden shadow-lg border bg-black aspect-[16/9] md:aspect-[21/9] max-h-[560px]">
            {youtubeId ? (
        ...
        {dualBanners.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            {dualBanners.map((banner, index) => {
              ...
              const bannerImage = (
                <div className="group relative w-full rounded-xl md:rounded-2xl overflow-hidden border bg-muted/40 aspect-[16/9] shadow-md hover:shadow-xl transition-all duration-300">
